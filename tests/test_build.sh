...
for ((arg_index=0; arg_index<${#ARGS[@]}; arg_index++)); do
  if [[ "${ARGS[$arg_index]}" == "-o" ]]; then
    OUT_FILE="${ARGS[$((arg_index+1))]}"
    break
  fi
done
...
if ! BUILD_SCHEMA_URL="https://api.butterbase.ai/v1/app_kn6g8399drob/fn/schemafetch?test=1" \
  ...
  CURL_BIN="$MOCK_CURL" \
  RUN_CONNECTIVITY_PREFLIGHT=0 \
  "$BUILD_SCRIPT" >"$LOG_FILE" 2>&1; then
  cat "$LOG_FILE" >&2
  echo "Build script should have succeeded with mock curl fallback but failed" >&2
  exit 1
fi
